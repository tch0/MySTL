#ifndef TUNORDERED_SET_HPP
#define TUNORDERED_SET_HPP
...
#include <tunordered_multiset.hpp>
...
template<typename Key, typename Hash = std::hash<Key>, typename KeyEqual = std::equal_to<Key>, typename Allocator = tstd::allocator<Key>,
    typename UnderlyingHashTable = tstd::impl::hash_table<Key, Key, tstd::impl::identity, false, Hash, KeyEqual, Allocator>>
class unordered_set;

template<typename Key, typename Hash, typename KeyEqual, typename Allocator, typename UnderlyingHashTable>
bool operator==(const tstd::unordered_set<Key, Hash, KeyEqual, Allocator, UnderlyingHashTable>& lhs, const tstd::unordered_set<Key, Hash, KeyEqual, Allocator, UnderlyingHashTable>& rhs);
template<typename Key, typename Hash, typename KeyEqual, typename Allocator, typename UnderlyingHashTable>
bool operator!=(const tstd::unordered_set<Key, Hash, KeyEqual, Allocator, UnderlyingHashTable>& lhs, const tstd::unordered_set<Key, Hash, KeyEqual, Allocator, UnderlyingHashTable>& rhs);
...
class unordered_set
{
    friend bool operator== <Key, Hash, KeyEqual, Allocator, UnderlyingHashTable>(const tstd::unordered_set<Key, Hash, KeyEqual, Allocator, UnderlyingHashTable>& lhs, const tstd::unordered_set<Key, Hash, KeyEqual, Allocator, UnderlyingHashTable>& rhs);
    friend bool operator!= <Key, Hash, KeyEqual, Allocator, UnderlyingHashTable>(const tstd::unordered_set<Key, Hash, KeyEqual, Allocator, UnderlyingHashTable>& lhs, const tstd::unordered_set<Key, Hash, KeyEqual, Allocator, UnderlyingHashTable>& rhs);
public:
    using key_type = Key;
...
public:
    // constructors
    unordered_set() // 1
        : unordered_set(initial_bucket_size)
    {
    }
    explicit unordered_set(size_type bucket_count, const Hash& hash = Hash(), const KeyEqual& equal = KeyEqual(), const Allocator& alloc = Allocator()) // 2
        : table(bucket_count, hash, equal, alloc)
    {
    }
    unordered_set(size_type bucket_count, const Allocator& alloc) // 3
        : unordered_set(bucket_count, Hash(), KeyEqual(), alloc)
    {
    }
    unordered_set(size_type bucket_count, const Hash& hash, const Allocator& alloc) // 4
        : unordered_set(bucket_count, hash, KeyEqual(), alloc)
    {
    }
    explicit unordered_set(const Allocator& alloc) // 5
        : unordered_set(initial_bucket_size, Hash(), KeyEqual(), alloc)
    {
    }
    template<typename InputIterator>
    unordered_set(InputIterator first, InputIterator last, size_type bucket_count = initial_bucket_size,
    ...
    template<typename InputIterator>
    unordered_set(InputIterator first, InputIterator last, size_type bucket_count, const Allocator& alloc) // 7
        : unordered_set(first, last, bucket_count, Hash(), KeyEqual(), alloc)
    {
    }
    template<typename InputIterator>
    unordered_set(InputIterator first, InputIterator last, size_type bucket_count, const Hash& hash, const Allocator& alloc) // 8
        : unordered_set(first, last, bucket_count, hash, KeyEqual(), alloc)
    {
    }
    unordered_set(const unordered_set& other) // 9
        : table(other.table, other.get_allocator())
    {
    }
    unordered_set(const unordered_set& other, const Allocator& alloc) // 10
        : table(other.table, alloc)
    {
    }
    unordered_set(unordered_set&& other) // 11
        : table(std::move(other.table))
    {
    }
    unordered_set(unordered_set&& other, const Allocator& alloc) // 12
        : table(std::move(other.table), alloc)
    {
    }
    unordered_set(std::initializer_list<value_type> il, size_type bucket_count = initial_bucket_size,
        const Hash& hash = Hash(), const KeyEqual& equal = KeyEqual(), const Allocator& alloc = Allocator()) // 13
        : unordered_set(il.begin(), il.end(), bucket_count, hash, equal, alloc)
    {
    }
    unordered_set(std::initializer_list<value_type> il, size_type bucket_count, const  Allocator& alloc) // 14
        : unordered_set(il, bucket_count, Hash(), KeyEqual(), alloc)
    {
    }
    unordered_set(std::initializer_list<value_type> il, size_type bucket_count, const Hash& hash, const Allocator& alloc) // 15
        : unordered_set(il, bucket_count, hash, KeyEqual(), alloc)
    {
    }
    ~unordered_set()
    ...
    // assignment
    unordered_set& operator=(const unordered_set& other) // 1
    {
        table = other.table;
        return *this;
    }
    unordered_set& operator=(unordered_set&& other) // 2
    ...
    unordered_set& operator=(std::initializer_list<value_type> il) // 3
    {
        table = il;
        return *this;
    }
...
// global swap for tstd::unordered_set
template<typename Key, typename Hash, typename KeyEqual, typename Allocator, typename UnderlyingHashTable>
void swap(tstd::unordered_set<Key, Hash, KeyEqual, Allocator, UnderlyingHashTable>& lhs, tstd::unordered_set<Key, Hash, KeyEqual, Allocator, UnderlyingHashTable>& rhs) noexcept(noexcept(lhs.swap(rhs)))
{
    lhs.swap(rhs);
}
